...
class Rate(SQLModel, table=True):
    ...
    job_id: uuid.UUID = Field(default=uuid.uuid4, foreign_key="jobs.id")
...
class Location(SQLModel, table=True):
    ...
    job_id: uuid.UUID = Field(default_factory=uuid.uuid4, foreign_key="jobs.id")
...
class Job(SQLModel, table=True):
    ...
    rate: Optional[Rate] = Relationship(back_populates="jobs")
    location: Optional[Location] = Relationship(back_populates="jobs")
    skills: List[Skill] = Relationship(back_populates="jobs")
...
Rate.job = Relationship(back_populates="rates")
Location.job = Relationship(back_populates="locations")
